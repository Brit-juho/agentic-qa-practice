...
diff의 변경 파일을 살펴보고 *파일 종류*를 식별. 다음 표에 해당하면
**관련 파일도 Grep/Read로 적극 확인**한 뒤 리뷰에 반영하세요. 단순 diff만
보면 *연쇄 영향*을 놓칩니다.

| 변경 파일이 ~ 이면 | 함께 확인할 것 |
|---|---|
| `models/*.py` (DB 모델/스키마) | 마이그레이션, 사용하는 service, 관련 쿼리, DTO |
| `routers/*.py` / `pages/*.tsx` | 호출되는 service, 같은 도메인 다른 라우터의 권한 패턴 |
| `services/*.py` | 호출하는 모든 라우터, 호출하는 다른 service |
| `auth.py` / `permission.*` | 권한을 사용하는 모든 라우터/페이지 |
| `api/client.ts` / `api/*` | 사용하는 모든 컴포넌트/페이지 |
| `*.config.*` / `package.json` / `pyproject.toml` | 영향 받는 빌드/런타임, 새 의존성의 보안 이슈 |
| 인터페이스 / 추상 클래스 | 모든 implementer (`grep "extends|implements"`) |
| 공통 유틸 / 헬퍼 | 사용하는 모든 호출 지점 |

확장 분석에서 발견한 *연쇄 영향*은 분석 요청의 차원 4 (Cascading Impact)
섹션에 명시적으로 기록하세요.

확장 *깊이*는 자율적으로 판단:
- 변경 5줄 미만 + 외부 영향 적으면 → 가벼운 확장 (1~2 파일)
- 변경이 크거나 모델/인터페이스/공통 → 깊은 확장 (5+ 파일)
- *시니어 엔지니어처럼* 의심되는 영향 영역을 *적극적으로* 추적
...
6. **위험 구역 자동 확장** — 변경 파일이 model/router/auth/공통 유틸 등
   *연쇄 영향이 큰 종류*면 관련 파일을 자동 Read/Grep으로 추적.
   시니어 엔지니어가 머릿속으로 하는 "이거 영향 어디까지 가지?" 판단을
   리뷰어 에이전트가 자동 수행.
